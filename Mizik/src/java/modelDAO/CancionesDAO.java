...
import model.Canciones;
    ...
    public List<Canciones> buscarCancionesPorNombre(String nombre) {
        String sql = "delete from Canciones where nombreCancion LIKE ?";
        List<Canciones> cancionEncontrados = new ArrayList<>();
        try {
            con = conect.getConnection();
            ps = con.prepareStatement(sql);
            ...
            while (rs.next()) {
                ...
                nCanciones.setCodigoArtista(rs.getInt("codigoArtista"));
                nCanciones.setCodigoAlbum(rs.getInt("codigoAlbum"));
                cancionEncontrados.add(nCanciones);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        return cancionEncontrados;
    }
