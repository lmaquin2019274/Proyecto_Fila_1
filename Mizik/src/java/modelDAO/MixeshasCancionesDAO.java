...
public class MixeshasCancionesDAO implements CRUDMixesHasCanciones {
    ...
    public List<MixeshasCanciones> listarMixeshasCanciones() {
       ...
       //       MixeshasCanciones.sethoraMC(rs.LocalDateTime ("horaMC"));
    ...
    public boolean agregarMixeshasCanciones(MixeshasCanciones per) {
        String sql = "insert into MixeshasCanciones(codigoMixeshasCanciones, fechaMC, horaMC, numeroUnico, codigoCancion, codigoMix) 
                       values('"+per.getCodigoMixeshasCanciones()+"', '"+per.getFechaMC()+"', '"+per.getHoraMC()+"', '"+per.getNumeroUnico()+"', '"+per.getCodigoCancion()+"',
                           '"+per.getCodigoMix()+"')";
        try{
            con = conect.getConnection();
            ps = con.prepareStatement(sql);
    ...
    @Override
    public Artistas buscarMixeshasCanciones(int id) {
        ...
        try {
          ...
          //    nMixeshasCanciones.sethoraMC(rs.LocalDateTime("fechaNacimiento"));
