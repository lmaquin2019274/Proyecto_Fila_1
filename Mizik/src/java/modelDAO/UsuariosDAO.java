...
public class UsuariosDAO implements CRUDUsuarios {
    ...
    public Usuarios validar(String usuario, String claveUsuario) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                user.setCorreoA(rs.getString("correoUsuario"));
                user.setNombreA(rs.getString("nombreCompleto"));
                x = true;
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResultSet fijarImagen(int codigoEmpleado) {

        String sql = "Select fotoPerfil from Usuarios where codigoUsuario = ?";
        try {
            con = conect.getConnection();
            ps = con.prepareStatement(sql);
