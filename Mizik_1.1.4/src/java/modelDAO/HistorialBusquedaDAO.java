...
public class HistorialBusquedaDAO implements CRUDHistorialBusqueda {
    ...
    public List<HistorialBusqueda> listarHistorialBusqueda() {
        ...
        String sql = ("select * from HistorialBusqueda where codigoUsuario =" +id);
        try {
            ...
            while (rs.next()) {
                ...
                LocalDateTime localDateTime = LocalDateTime.of(LocalDate.now(), rs.getTime("horaHB").toLocalTime()); // tenia error y lo corregi
                nuevocodigoHistorialB.setHoraHB(localDateTime);
