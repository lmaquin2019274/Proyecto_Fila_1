...
public class HistorialReproduccionDAO implements CRUDHistorialReproduccion {
    ...
    public List<HistorialReproduccion> listarHistorialReproduccion() {
        ...
        String sql = "select * from HistorialReproduccion where codigoUsuario = "+id;
        try {
            con = conect.getConnection();
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
                ...
                nHistorialR.setHoraHR(localDateTime); // corregi la hora
                nHistorialR.setOrigen(rs.getString("origen"));
                nHistorialR.setCodigoUsuario(rs.getInt("codigoUsuario"));
//                nHistorialR.setCodigoCancion(rs.getInt("codigoCancion")); EN LA BASE DE DATOS NO AGREGARON ESTO
